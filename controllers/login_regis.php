...
if (!empty($_POST['phoneNum']) && !empty($_POST['email']) && !empty($_POST['user']) && !empty($_POST['pass'])) {
    try {
        //set data from the database
        $sql = "INSERT INTO users (users_phoneNumber, users_email, users_name, users_pass, users_status_id, users_dateTime) 
        VALUE ('{$_POST['phoneNum']}', '{$_POST['email']}', '{$_POST['user']}', '{$_POST['pass']}', 0, CURRENT_TIMESTAMP) ";
        $conn->query($sql);
